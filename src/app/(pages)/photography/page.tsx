  ...
  return (
    <section className="pt-32 pb-24">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center justify-between mb-4"><span className="inline-block px-3 py-1 rounded-full bg-[#1A1A1A]/8 text-[#1A1A1A] text-xs font-semibold tracking-wider uppercase">Photography</span><a href="https://www.pexels.com/@sam-lee-2162121365/" target="_blank" rel="noopener" className="text-xs text-[#666] hover:text-[#d97706] transition-colors underline underline-offset-2">Pexels ↗</a></div>
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">{t("photoTitle")}</h1>
        <p className="text-lg text-[#666] mb-10 max-w-xl">{t("photoDesc")}</p>
        <div className="flex gap-2 mb-10 flex-wrap">{catKeys.map(k=><button key={k} onClick={()=>setCat(k)} className={"px-5 py-2 rounded-full text-sm font-medium transition-all "+(cat===k?"bg-[#d97706] text-white":"bg-transparent border border-[#1A1A1A]/8 text-[#666] hover:border-[#d97706]")}>{pc[k]||k}</button>)}</div>
