...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-40 border-b" style={{background:"rgba(246,243,236,0.9)",backdropFilter:"blur(16px)",borderColor:"rgba(26,26,26,0.04)"}}>
          ...
          {links.map(k => {
            ...
            return (
              <Link key={k} href={hrefs[k]}
                    ...
                    style={isActive ? {color:"#1A1A1A"} : {color:"#999"}}>
                {t("nav."+k)}
              </Link>
            );
          })}
      ...
      {open && (
        <div className="md:hidden px-6 pb-6 space-y-2" style={{background:"#F6F3EC"}}>
          {links.map(k => (
            <Link key={k} href={hrefs[k]} onClick={()=>setOpen(false)}
                  className="block py-2 mono text-xs tracking-[0.1em] uppercase"
                  style={pathname===hrefs[k] ? {color:"#1A1A1A"} : {color:"#999"}}>
              {t("nav."+k)}
            </Link>
          ))}
          <button onClick={toggle}
